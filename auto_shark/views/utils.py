...
from auto_shark import mail
from auto_shark import app_root
...
def save_video(form_video):
    ...
    video_path = os.path.join(app_root, 'static/videos/', video_fn)

    output_size = (3024, 4032)
    i = Image.open(form_video)
    rgb_i = i.convert('RGB')
    rgb_i.thumbnail(output_size)
    rgb_i.save(video_path)

    return video_fn


def upload_video(form_video):
